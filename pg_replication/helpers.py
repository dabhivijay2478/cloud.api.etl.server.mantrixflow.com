...
class ItemGenerator:
    ...
    table_name_map: Optional[Dict[str, str]] = (None,)  # type: ignore[assignment]
...
class MessageConsumer:
    ...
    def process_relation(self, decoded_msg: Relation) -> None:
        ...
        source_table_name = decoded_msg.relation_name
        table_name = self.table_name_map.get(source_table_name, source_table_name)
        ...
        include_columns = (
            ...
            else self.include_columns.get(source_table_name)
        )
        if include_columns is not None:
            columns = {k: v for k, v in columns.items() if k in include_columns}
        # 2) override source hints
        column_hints: TTableSchemaColumns = (
            dict() if self.columns is None else self.columns.get(source_table_name, dict())
        )
