...
class PostgresConnector(SQLConnector):
    ...
    def jsonschema_to_sql(self) -> JSONSchemaToSQL:
        ...
        # x-sql-datatype handlers for output_column_sql_types (avoids UUID->TEXT mismatch)
        to_sql.register_sql_datatype_handler("uuid", lambda _: UUID())
        to_sql.register_sql_datatype_handler("text", TEXT)
        to_sql.register_sql_datatype_handler("character varying", lambda _: VARCHAR())
        to_sql.register_sql_datatype_handler("varchar", lambda _: VARCHAR())
        to_sql.register_sql_datatype_handler("boolean", BOOLEAN)
        to_sql.register_sql_datatype_handler("jsonb", JSONB)
        to_sql.register_sql_datatype_handler("date", DATE)
        to_sql.register_sql_datatype_handler(
            "timestamp with time zone", lambda _: TIMESTAMP()
        )
        to_sql.register_sql_datatype_handler(
            "timestamp without time zone", lambda _: TIMESTAMP()
        )
        to_sql.register_sql_datatype_handler("double precision", lambda _: sa.REAL())
        to_sql.register_sql_datatype_handler("real", lambda _: sa.REAL())
        to_sql.register_sql_datatype_handler("numeric", lambda _: DECIMAL())
        return to_sql
    ...
    def merge_sql_types(
        ...
        """Override SDK merge to use Postgres-aware type precedence.

        The SDK's default merge_sql_types ranks types by their Python
        representation (python_type). This causes postgresql.UUID to rank
        below TEXT because ``postgresql.UUID().python_type`` is ``uuid.UUID``
        (rank 0) while ``TEXT().python_type`` is ``str`` (rank 900).

        The result is that when a destination table already has a UUID column
        and the incoming Singer schema says TEXT (which tap-postgres emits when
        it lacks a UUID format handler), merge_sql_types returns TEXT, triggering
        a UUID→TEXT ALTER that fails with allow_column_alter=False.

        This override uses the explicit PostgreSQL precedence list in
        pick_best_sql_type, where UUID outranks TEXT, so UUID columns are
        preserved correctly.

        Args:
            sql_types: List of SQL types to merge.

        Returns:
            The best SQL type according to Postgres precedence.

        Raises:
            ValueError: If sql_types is empty.
        """
        if not sql_types:
            msg = "Expected at least one member in `sql_types` argument."
            raise ValueError(msg)
        return PostgresConnector.pick_best_sql_type(sql_type_array=list(sql_types))
    ...
    def _adapt_column_type(  # type: ignore[override]  # noqa: PLR0913
        ...
        # If both the existing column type and the merged type are string-family
        # (TEXT, VARCHAR, CHAR, etc.) they are functionally compatible in PostgreSQL —
        # all string variants are stored identically under the hood.  Attempting to
        # ALTER between them is both unnecessary and unsupported (allow_column_alter
        # is False), so we skip the alteration entirely.
        try:
            if isinstance(current_type.as_generic(), sa.types.String) and isinstance(
                compatible_sql_type.as_generic(), sa.types.String
            ):
                return
        except NotImplementedError:
            # as_generic() raises NotImplementedError for some custom types;
            # fall through to normal handling in that case.
            pass
